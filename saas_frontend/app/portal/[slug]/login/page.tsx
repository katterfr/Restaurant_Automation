...
export default function SlugLoginPage() {
  ...
  const [newAccount, setNewAccount] = useState<{ google_email?: string; google_name?: string; google_id?: string; phone?: string } | null>(null)
  ...
  async function handleGoogleCredential(response: { credential: string }) {
    ...
    try {
      ...
      if ('access_token' in result) {
        ...
        setNewAccount({ google_email: result.google_email, google_name: result.google_name, google_id: result.google_id })
        setStep('link-google')
      }
    } catch (e: unknown) {
  ...
  async function handleSendOtp(e: React.FormEvent) {
    ...
    try {
      ...
      setNewAccount({ phone: result.phone })
      setStep(result.linked ? 'otp' : 'otp')
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    setError('')
    try {
      const result = await authApi.verifyPhoneOtp(newAccount?.phone || phone, otp)
      if ('access_token' in result) {
        await afterLogin(result.access_token)
      } else {
        setStep('link-phone')
  ...
  async function handleLinkPhone(e: React.FormEvent) {
    ...
    setError('')
    try {
      const result = await authApi.phoneLink(newAccount?.phone || phone, otp, email, password)
      await afterLogin(result.access_token)
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Linking failed')
    } finally {
      ...
      <div className="w-full max-w-sm px-4">
          ...
          <div className="space-y-3">
            ...
            {process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID ? (
              ...
              <button disabled className={`${btnCls} opacity-40 cursor-not-allowed`}>
                <GoogleIcon /> Continue with Google <span className="text-xs text-gray-400">(configure in Vercel)</span>
              </button>
            )}
            ...
            {/* Apple (placeholder) */}
            <button
              onClick={() => setError('Apple Sign-In requires Apple Developer setup. Contact your admin.')}
              className={btnCls}
            >
              <AppleIcon />
              Continue with Apple
            ...
            <p className="text-center text-xs text-gray-400 pt-2">
              ...
              <Link href={`/portal/${slug}/forgot-password`} className="text-green-700 hover:underline font-medium">Reset it here</Link>
            </p>
          </div>
        )}
        ...
        {step === 'email' && (
          <form onSubmit={handleEmailLogin} className="space-y-4">
            <button type="button" onClick={() => setStep('options')} className="text-sm text-gray-400 hover:text-gray-600 mb-1">← Back</button>
            <div>
        ...
        {step === 'phone' && (
          <form onSubmit={handleSendOtp} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1.5">We'll send you a verification code.</p>
            </div>
            {error && <p className="text-red-600 text-sm bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</p>}
        ...
        {step === 'otp' && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div className="text-center mb-2">
              <p className="text-sm text-gray-600">{info || `Code sent to ${newAccount?.phone}`}</p>
            </div>
            <div>
        ...
        {/* ── Step: link Google to existing account ────────────────────── */}
        {step === 'link-google' && (
          <form onSubmit={handleLinkGoogle} className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800">
              ...
              <p className="text-blue-600 text-xs">Signed in as {newAccount?.google_email}. Enter your Careful Server credentials to link.</p>
            </div>
            <div>
        ...
        {/* ── Step: link phone to existing account ─────────────────────── */}
        {step === 'link-phone' && (
          <form onSubmit={handleLinkPhone} className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800">
              <p className="font-medium mb-0.5">Link your phone number</p>
              <p className="text-blue-600 text-xs">No account found for {newAccount?.phone}. Enter your Careful Server credentials to link this number.</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Portal email</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus className={inputCls} placeholder="owner@restaurant.com" />
